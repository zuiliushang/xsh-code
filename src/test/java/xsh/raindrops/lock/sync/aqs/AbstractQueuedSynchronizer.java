...
/**
	感谢 {@link http://www.cnblogs.com/waterystone/p/4920797.html}
	...
	2.AQS内部组成?
		维护了一个 volatile int state（代表共享资源）和一个 FIFO 的线程等待队列
		state : getState
				setState
				compareAndSetState
	AQS 定义了两种资源共享方式:
		Exclusive (资源独占,只有一个线程能执行,如 {@link ReentrantLock})
		Share(共享资源,多个线程可同时执行,如 {@link Semaphore}/{@link CountDownLatch})
...
*****************************************************************************************
	...
	tryAcquire(int)：独占方式。尝试获取资源，成功则返回true，失败则返回false。
 	...
 	再以CountDownLatch以例，任务分为N个子线程去执行，
 		state也初始化为N（注意N要与线程个数一致）。
 		这N个子线程是并行执行的，每个子线程执行完后countDown()一次，state会CAS减1。
 		等到所有子线程都执行完后(即state=0)，会unpark()主调用线程，然后主调用线程就会从await()函数返回，
 		继续后余动作。
